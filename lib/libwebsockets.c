...
lws_sum_stats(const struct lws_context *ctx, struct lws_conn_stats *cs)
{
	...
	while (vh) {
		...
		cs->http2_alpn += vh->conn_stats.http2_alpn;
		cs->http2_subs += vh->conn_stats.http2_subs;
		...
		int hide_vhosts)
{
	...
	buf += lws_snprintf(buf, end - buf,
			...
			" \"http2_subs\":\"%lu\",\n"
			" \"http2_upg\":\"%lu\"",
			listening, cs.rx, cs.tx, cs.conn, cs.trans,
			cs.ws_upg, cs.rejected, cs.http2_alpn, cs.http2_subs,
			cs.http2_upg);
